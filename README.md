...
[项目主页](https://github.com/3495445484-cpu)
...
- `.env` 和 `.env.*` 已被 `.gitignore` 忽略，不要提交真实 API Key。
- 仓库只保留 `.env.example` 占位示例，其他使用者需要配置自己的 Key。
- 如果没有配置真实 Key，后端仍可启动，但 AI 咨询会进入本地 fallback 模式。
